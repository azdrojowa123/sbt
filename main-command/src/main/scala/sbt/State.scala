        ...
        case x :: xs =>
          (x.execId, x.source) match {
            ...
            case (Some(id), Some(s))
              ...
              val map = Exec(s"$MapExec $id $newID", x.source)
              val complete = Exec(s"$CompleteExec $id", x.source)
              val report = Exec(s"$ReportResult $id", Some(id), x.source)
              val stash = Exec(StashOnFailure, x.source)
              val failureWall = Exec(FailureWall, x.source)
              val pop = Exec(PopOnFailure, x.source)
